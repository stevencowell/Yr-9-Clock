(function () {
  ...
  const STORAGE_KEY = config.storageKey || 'clock-guided-lesson';
  ...
  function downloadPdf() {
    ...
    const fileName = `${(state.studentName || 'student').replace(/[^a-z0-9]+/gi, '-').replace(/^-|-$/g, '')}-${(config.title || 'clock').replace(/[^a-z0-9]+/gi, '-').replace(/^-|-$/g, '')}.pdf`;
